...
function generator(adj, noun) {
  ...
  // var nouns = getNoun(noun); //0.3
  var nouns = nounsCollection[noun]; //0.3
  // var randomAdjective = parseInt(Math.random() * adjectives.length);
  // var randomNoun = parseInt(Math.random() * nouns.length);
  return "The " + adjectives[parseInt(Math.random() * adjectives.length)].capitalize() + " " + nouns[parseInt(Math.random() * nouns.length)].capitalize();;
}
...
var pizzaElementGenerator = function(i) {
  ...
  pizzaName.innerHTML = randomName(); //6.8ms //2ms
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = documentPosition + (i % 5);
