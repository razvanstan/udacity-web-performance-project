...
function generator(adj, noun) {
  var adjectives = adjCollection[adj];
  var nouns = nounsCollection[noun];
  return "The " + adjectives[parseInt(Math.random() * adjectives.length)] + " " + nouns[parseInt(Math.random() * nouns.length)];
}
...
var pizzaGenerator = function(i) {
    return '<div id="pizza'+i+'" class="randomPizzaContainer"><div class="randomPizzaImageContainer"><img src="images/pizza.png" class="img-responsive"></div><div style="width:65%"><h4>'+randomName()+'</h4><ul>'+makeRandomPizza()+'</ul></div></div>';
};
...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
      $('.randomPizzaContainer')
          .removeClass('size-1 size-2 size-3')
          .addClass('size-'+size);
  }
...
for (var i = 2; i < 100; i++) {
    pizzasTemplate += pizzaGenerator(i);
}

$('#randomPizzas').append(pizzasTemplate);
...
function updatePositions() {
    ...
    var documentPosition = Math.sin((document.body.scrollTop / 1250));
